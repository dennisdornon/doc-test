...
#  libs/phpseclib/vendor/phpseclib/phpseclib/phpseclib/Crypt AES.php
...
If {@link self::setKeyLength()} isn't called, it'll be calculated from {@link self::setKey()}. ie. if the key is 128-bits, the key length will be 128-bits. If it's 136-bits it'll be null-padded to 192-bits and 192 bits will be the key length until {@link self::setKey()} is called, again, at which point, it'll be recalculated.

Since \phpseclib3\Crypt\AES extends \phpseclib3\Crypt\Rijndael, some functions are available to be called that, in the context of AES, don't make a whole lot of sense. {@link self::setBlockLength()}, for instance. Calling that function, however possible, won't do anything (AES has a fixed block length whereas Rijndael has a variable one).
...
[AES](classes/phpseclib3-Crypt-AES.md) |  _Pure-PHP implementation of AES._  
---|---  
