...
#  libs/phpseclib/vendor/phpseclib/phpseclib/phpseclib/Crypt Rijndael.php
...
If {@link self::setBlockLength()} isn't called, it'll be assumed to be 128 bits. If {@link self::setKeyLength()} isn't called, it'll be calculated from {@link self::setKey()}. ie. if the key is 128-bits, the key length will be 128-bits. If it's 136-bits it'll be null-padded to 192-bits and 192 bits will be the key length until {@link self::setKey()} is called, again, at which point, it'll be recalculated.

Not all Rijndael implementations may support 160-bits or 224-bits as the block length / key length. mcrypt, for example, does not. AES, itself, only supports block lengths of 128 and key lengths of 128, 192, and 256. {@link http://csrc.nist.gov/archive/aes/rijndael/Rijndael-ammended.pdf#page=10} defines the algorithm for block lengths of 192 and 256 but not for block lengths / key lengths of 160 and 224. Indeed, 160 and 224 are first defined as valid key / block lengths in {@link http://csrc.nist.gov/archive/aes/rijndael/Rijndael-ammended.pdf#page=44}: Extensions: Other block and Cipher Key lengths. Note: Use of 160/224-bit Keys must be explicitly set by setKeyLength(160) respectively setKeyLength(224).
...
[Rijndael](classes/phpseclib3-Crypt-Rijndael.md) |  _Pure-PHP implementation of Rijndael._  
---|---  
